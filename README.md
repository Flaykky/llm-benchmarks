...
* **Correctness & Functional Tests (40 pts)**
  ...
  * 0–40 proportionally to passing unit/integration tests.
* **Performance & Resource Limits (20 pts)**

  * Time complexity, memory bound, throughput vs baseline. (If the task is not performance-critical, this becomes up to 10 pts and remaining points redistributed to code quality.)
* **Security & Robustness (15 pts)**

  * Input validation, safe handling of edge cases, resistance tests.
* **Adherence to Constraints (10 pts)**

  * Follow "no recursion", "functional style", language-specific constraints.
* **Code Quality, Readability & Comments (10 pts)**

  * Naming, modularity, helpful comments explaining non-obvious choices.
* **Tests & Reproducibility (5 pts)**
...
* **55–74 — Advanced**: Correct on many tasks, good performance, sometimes misses edge-cases.
...
4. Check constraint adherence automatically (grep for banned APIs or recursion markers).
5. Review comments and threat-model explanations for security tasks.
6. For extremely long tasks, ask the model to produce modularized output (but remember: models can’t run asynchronous work — score what’s delivered).
...
* Encourage models to include minimal tests (unit tests) inside the message to demonstrate correctness.
